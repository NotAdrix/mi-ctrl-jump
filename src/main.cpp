...
class $modify(CCKeyboardDispatcher) {
    bool dispatchKeyboardMSG(enumKeyCodes key, bool down, bool isRepeat) {
        ...
        if (key == key1 || key == key2) {
            return CCKeyboardDispatcher::dispatchKeyboardMSG(enumKeyCodes::KEY_Space, down, isRepeat);
        }
...
$execute {
    geode::MouseInputEvent().listen([](geode::MouseInputData data) -> ListenerResult {
        ...
        if (data.button == geode::MouseInputData::Button::Right) {
            kbd->dispatchKeyboardMSG(enumKeyCodes::KEY_Z, down, false);
            return ListenerResult::Stop;
        }

        if (data.button == geode::MouseInputData::Button::Middle) {
            kbd->dispatchKeyboardMSG(enumKeyCodes::KEY_X, down, false);
            return ListenerResult::Stop;
        }
